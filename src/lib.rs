...
#[no_mangle]
extern "C" fn handle() {
    let game_state = unsafe { PEBBLES_GAME.as_mut().expect("Failed to load game state") };
    ...
    match action {
        PebblesAction::Turn(count) => match remove_pebbles(count) {
            Some(()) => {
...
#[no_mangle]
extern "C" fn state() {
    let game_state = unsafe { PEBBLES_GAME.take().expect("Game state is not initialized") };
...
fn exec_program_turn() -> u32 {
    let game_state: &mut GameState =
        unsafe { PEBBLES_GAME.as_mut().expect("Failed to load game state") };
    let count: u32;
...
fn remove_pebbles(count: u32) -> Option<()> {
    let game_state = unsafe { PEBBLES_GAME.as_mut().expect("Failed to load game state") };
